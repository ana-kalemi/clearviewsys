...
function success_load(xml) {
    ...
    $(xml).find('RATE').each(function () {
        ...
        if ($(this).find('ISFLAGGED').text() === '**') {
            ...
            if (String(inv_sell).length <= 5 && Math.ceil(inv_sell) - inv_sell > 0) {
                inv_sell = inv_sell + '0';
            }
            if (String(inv_buy).length <= 5 && Math.ceil(inv_buy) - inv_buy > 0) {
                inv_buy = inv_buy + '0';
            }
        ...
        if ($(this).find('ISFLAGGED').text() == '**' && horizontalScroll == true) {
            rateLine += '<div class="rate-row">' +
                ...
                '<span class="horizontal_rate" >Buy: ' + '<span class="horizontal_buy">'+$(this).find('WEBUY').html() + '</span></span>' +

...
/**
 * rates quantity in left table, by default 10, you can change it.
 * */
var qty = 10;
...
function success_load(xml) {
    ...
    /**
     * create HTML for rates board table
     * */
    var i = 0;
    var tableSelector;
    $(xml).find('RATE').each(function () {
        ...
        if(i <= (qty-1)){
            tableSelector = '.main_tbody.left-table';
        }else{
            tableSelector = '.main_tbody.right-table';
        }

        if ($(this).find('ISFLAGGED').text() == '**') {
            $(tableSelector).append('<tr class="row_currencyUp">' +
    ...
    /**
     * Column option - remove columns that have display option = false
     * */
    if (displayColFlag == false) {

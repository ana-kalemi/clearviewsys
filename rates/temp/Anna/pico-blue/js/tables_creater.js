...
function success_load(xml) {
    ...
    $(xml).find('RATE').each(function () {
        ...
        if ($(this).find('ISFLAGGED').text() === '*') {
            ...
            if (rowQty < maxSliderows) {
                ...
                firstSlide += prepare_table_row(img, countryStr, code_name, currency, we_buy, we_sell, inv_buy, inv_sell);
            } else {
                /**add row to slider rows string*/
                secondSlide += prepare_table_row(img, countryStr, code_name, currency, we_buy, we_sell, inv_buy, inv_sell);
            }
            rowQty++;
        }
    ...
    $('#main-rates .main_tbody').html(rateRows);
    $('#first_slide_table .main_tbody').html(firstSlide);
    $('#second_slide_table .main_tbody').html(secondSlide);

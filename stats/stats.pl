...
sub html_head {
	if (scalar keys %HTMLOutput || $PluginMode) {
		...
		if ($UseXml) {
            ...
            if ($FrameName ne 'index') { print "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Strict//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd\">\n";  }
			else { print "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Frameset//EN\" \"https://www.w3.org/TR/xhtml1/DTD/xhtml1-frameset.dtd\">\n"; }
			print "<html xmlns=\"https://www.w3.org/1999/xhtml\" xml:lang=\"$Lang\">\n";
		} else {
...
sub html_end {
	if (scalar keys %HTMLOutput) {
		if ($FrameName ne 'index' && $FrameName ne 'mainleft') {
			...
			print "<b>Advanced Web Statistics $VERSION</b> - <a href=\"https://awstats.sourceforge.net\" target=\"awstatshome\">Created by $PROG";
	...
	if (! $ErrorMessages && $message =~ /^Format error$/i) {
		...
		if ($message !~ $LogSeparator) {
			...
			if ($LogFormat == 1) {
				...
				print "111.22.33.44 - - [10/Jan/2001:02:14:14 +0200] \"GET / HTTP/1.1\" 200 1234 \"https://www.fromserver.com/from.htm\" \"Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)\"\n";
			...
			if ($LogFormat == 6) {
				...
				print (scalar keys %HTMLOutput?"$tagfontgrey<i>":"");
				print "111.22.33.44 - Firstname Middlename Lastname [10/Jan/2001:02:14:14 +0200] \"GET / HTTP/1.1\" 200 1234 \"https://www.fromserver.com/from.htm\" \"Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)\"\n";
...
sub Check_Config {
	...
	$LogoLink||='https://awstats.sourceforge.net';
			 ...
			 || ($versionnum < 5000 && $SectionsToLoad{"general"} && $FirstTime{$year.$month} && $LastTime{$year.$month}) )		{
				...
				if ($versionnum < 5000 && ! $MigrateStats && ! $BadFormatWarning{$year.$month}) {
					if ($FrameName ne 'mainleft') {
						...
						if ($ENV{'GATEWAY_INTERFACE'} && $AllowToUpdateStatsFromBrowser) { $message.="\nFrom your browser with URL: <a href=\"https://".$ENV{"SERVER_NAME"}.$ENV{"SCRIPT_NAME"}."?migrate=$filetoread\">https://".$ENV{"SERVER_NAME"}.$ENV{"SCRIPT_NAME"}."?migrate=$filetoread</a>"; }
						warning("$message");
					}
				}
				if (! ($versionnum < 5000) && $MigrateStats && ! $BadFormatWarning{$year.$month}) {
...
sub Save_History {
	...
	if ($sectiontosave eq 'sider') {	# This section must be saved after VISITOR section is read
		...
		foreach my $key (@keylist) {
			$keysinkeylist{$key}=1;
			my $newkey=$key;
			$newkey =~ s/([^:])\/\//$1\//g;		# Because some targeted url were taped with 2 / (Ex: //rep//file.htm). We must keep https://rep/file.htm
	...
	if ($sectiontosave eq 'pagerefs') {
		...
		foreach my $key (@keylist) {
			$keysinkeylist{$key}=1;
			my $newkey=$key;
			$newkey =~ s/^http(s|):\/\/([^\/]+)\/$/http$1:\/\/$2/i;	# Remove / at end of https://.../ but not at end of https://.../dir/
			$newkey =~ s/\s/%20/g;
...
sub DefinePerlParsingFormat {
	...
	# Apache combined: 62.161.78.73 user - [dd/mmm/yyyy:hh:mm:ss +0000] "GET / HTTP/1.1" 200 1234 "https://www.from.com/from.htm" "Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)"
	...
	# IIS: 2000-07-19 14:14:14 62.161.78.73 - GET / 200 1234 HTTP/1.1 Mozilla/4.0+(compatible;+MSIE+5.01;+Windows+NT+5.0) https://www.from.com/from.htm
	# WebStar: 05/21/00	00:17:31	OK  	200	212.242.30.6	Mozilla/4.0 (compatible; MSIE 5.0; Windows 98; DigExt)	https://www.cover.dk/	"www.cover.dk"	:Documentation:graphics:starninelogo.white.gif	1133
	# Squid extended: 12.229.91.170 - - [27/Jun/2002:03:30:50 -0700] "GET https://www.callistocms.com/images/printable.gif HTTP/1.1" 304 354 "-" "Mozilla/5.0 Galeon/1.0.3 (X11; Linux i686; U;) Gecko/0" TCP_REFRESH_HIT:DIRECT
...
if ((! $ENV{'GATEWAY_INTERFACE'}) && (! $SiteConfig)) {
	...
	print "New versions and FAQ at https://awstats.sourceforge.net\n";
	exit 2;
}
			...
			if ($field[$pos_referer] eq '-' || $field[$pos_referer] eq 'bookmarks') {	# "bookmarks" is sent by Netscape, '-' by all others browsers
				...
				if ($refererprot =~ /^http/i) {
					...
					if ($TmpRefererServer{$refererserver}) {
						...
						$_from_h[3]++;
						# https://www.mysite.com/ must be same referer than https://www.mysite.com but .../mypage/ differs of .../mypage
...
if (scalar keys %HTMLOutput) {
	...
	if ($ShowMenu || $FrameName eq 'mainleft') {
		...
		if ($FrameName ne 'mainright') {
			...
			# Logo and flags
			if ($FrameName ne 'mainleft') {
				if ($LogoLink =~ "https://awstats.sourceforge.net") {
